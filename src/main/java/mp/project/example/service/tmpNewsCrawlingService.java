...
@Service
public class tmpNewsCrawlingService {
    private final NewsRepository newsRepository;
    private final UserRepository userRepository;
    private final OpenAIService openAIService;
    public tmpNewsCrawlingService(NewsRepository newsRepository,UserRepository userRepository,OpenAIService openAIService) {
...
//일단은 하루치만 크롤링 나중에 연도별로 고를수있게 
        ...
        for (Element article : articles) {
            ...
            //  연예 및 스포츠 기사 건너뛰기
        if (link.contains("entertain") || link.contains("sports")) {
        System.out.println(" 동적 구조 기사 건너뜀: " + link);
        continue;
    }
            // 썸네일 이미지 추출
    ...
    private String extractNewsBody(String link) {
        try {
            Document newsDoc = Jsoup.connect(link).userAgent("Mozilla/5.0").get();
